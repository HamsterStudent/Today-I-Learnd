...
function reloadReviews(){
  $('#prdReview .review-title').each(function(index,e) {

      var no = $(e).attr('href').replace(/(\S*)[?&]no=(\d+)(\S*)/g, '$2');
      ...
      REVIEW.getReadData($(e));
  });
}

function openWindow(){

  const windowImg = document.querySelectorAll('.product-review-read .view .attach img')
  const modalWindow = document.querySelector('.review-modal-wrap')
  const modalImg = document.querySelector('.review-modal-img')

  for(let i = 0; i < windowImg.length; i++){
    windowImg[i].addEventListener('click', (e)=>{

      modalWindow.style.display = 'block'
      modalImg.innerHTML = ' <img src =" '+ e.target.src + ' " border="0" alt=""/> '
    })
  }
            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
                    ...
                    if (data.read['content_image'] != null) {
                        var sImg = data.read['content_image'].replace('/gallery','');
                    } else {
                        var sImg = '';
                    }
                ...
                $(pNode).append('<div class="product-review-read'+data.key+'" class="contentView">'+aHtml.join('')+'</div>');
